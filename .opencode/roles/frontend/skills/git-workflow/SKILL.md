---
name: git-workflow
description: Git 分支命名、提交规范、PR 模板。前端开发人员使用：只提交 PR 到 develop，不合并。
---
...
- `develop` 是集成分支：前端功能分支从 `develop` 拉出，PR 以 `develop` 为 base
...
```
前端开发人员（本角色）
  feat/frontend/xxx（从 develop 拉出）
  ├─ 契约驱动 + Mock，交付 docs/frontend/
...
|:-----|:----------|:-----------|:-----|
| `feat` | feat/frontend/todo-tags | feat: 添加标签功能 | 新功能 |
| `fix` | fix/frontend/login-crash | fix: 修复登录崩溃 | Bug 修复 |
| `hotfix` | hotfix/frontend/blank-page | hotfix: 修复白屏 | 线上紧急修复 |
| `refactor` | refactor/frontend/auth-module | refactor: 重构认证模块 | 重构，不改功能 |
| `perf` | perf/frontend/list-render | perf: 优化列表渲染性能 | 性能优化 |
| `chore` | chore/frontend/update-deps | chore: 升级依赖版本 | 杂项/维护 |

分支名全小写，单词用连字符分隔。前端分支统一带 `frontend` 作用域。

## Release 流程（前端视角）

```
feat/frontend/xxx → /pr → 结合人员合并到 develop
        ↓ 集成验证通过后
结合人员合并 develop → main
        ↓ 全部合入后

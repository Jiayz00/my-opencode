<!-- flow-v0.3 --> 以项目内版本为准（~/.config/opencode/docs/ 为全局副本，读取时校验版本标记，低于 v0.3 即 STOP 并用 /vibe-role 重装）
...
6. 提交 release → main 的 merge PR 并验证（需用户确认，人工 merge）
7. 创建 GitHub Release
8. 报告结果与回滚指引
...
- **扫描摘要**：按 `.opencode/roles/integration/docs/pr-flow.md`（缺失则 `~/.config/opencode/docs/pr-flow.md`）Step 3 的内容正则表扫描 git log 输出，命中即 **STOP**（防凭据经 CHANGELOG 泄漏到仓库与公开 Release 页）
- 更新 `CHANGELOG.md`：
...
- **提交前脱敏扫描（硬门禁，复用 `.opencode/roles/integration/docs/pr-flow.md`（缺失则 `~/.config/opencode/docs/pr-flow.md`）Step 3 完整扫描）**：
  - 2a 文件名扫描：按 `.opencode/roles/integration/docs/pr-flow.md`（缺失则 `~/.config/opencode/docs/pr-flow.md`）Step 3 的 blocklist 检查改动文件
  - 2b 内容扫描：按 `.opencode/roles/integration/docs/pr-flow.md`（缺失则 `~/.config/opencode/docs/pr-flow.md`）Step 3 的内容正则表检查改动内容
  - 任一命中 → **STOP**，处理后再继续
...
### 6. 合并回 main（提交 merge PR，人工 merge）

- **步骤内确认**：用 `question` 明确请求提交 merge PR 的许可，未确认不得继续
- 先推送 release 分支：
...
- **提交 merge PR**（守卫 main 铁律：合 main 只提交 merge PR，人工 merge）：

```bash
gh pr create --base main --head release/vX.Y.Z --title "merge: 发布 vX.Y.Z" --body "<CHANGELOG 摘要>"
```

- **Do NOT merge** —— 报告 PR URL，**merge 由人工在 GitHub 手动点击执行**
- 合并冲突 → 按 `git-workflow` skill 的冲突解决流程（SKILL.md:73-127）处理，禁止跳过；解决后重新测试再继续
- 合并被拒绝（冲突/保护规则拦截）→ 停止并报告，不绕过保护规则、不请求临时关闭分支保护
- **合并后验证**（人工 merge 完成后）：等待 CI 对 main 的检查通过（本仓库无 CI 则运行测试命令）；失败 → **停止**并报告，不得进入下一步

### 7. 创建 GitHub Release

- 提取 CHANGELOG 中 `## [vX.Y.Z]` 节内容作为 notes；无 CHANGELOG 时用 `--generate-notes` 生成草稿并**人工审阅后发布**
- **组装前再扫描**：按 `.opencode/roles/integration/docs/pr-flow.md`（缺失则 `~/.config/opencode/docs/pr-flow.md`）Step 3 的内容正则表对 notes 内容再校验，命中即 STOP
...
- **代码错误**：`git revert -m 1 <merge-commit>` 生成回滚提交，按 `.opencode/roles/integration/docs/pr-flow.md` 完整流程合回 main，发补丁版本

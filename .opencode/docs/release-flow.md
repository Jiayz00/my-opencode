...
- **扫描摘要**：按 `.opencode/docs/pr-flow.md`（缺失则 `~/.config/opencode/docs/pr-flow.md`）Step 2b 的内容正则表扫描 git log 输出，命中即 **STOP**（防凭据经 CHANGELOG 泄漏到仓库与公开 Release 页）
- 更新 `CHANGELOG.md`：
...
- **提交前脱敏扫描（硬门禁，复用 `.opencode/docs/pr-flow.md`（缺失则 `~/.config/opencode/docs/pr-flow.md`）Step 2 完整扫描）**：
  - 2a 文件名扫描：按 `.opencode/docs/pr-flow.md`（缺失则 `~/.config/opencode/docs/pr-flow.md`）Step 2a 的 blocklist 检查改动文件
  - 2b 内容扫描：按 `.opencode/docs/pr-flow.md`（缺失则 `~/.config/opencode/docs/pr-flow.md`）Step 2b 的内容正则表检查改动内容
  - 任一命中 → **STOP**，处理后再继续
...
- **组装前再扫描**：按 `.opencode/docs/pr-flow.md`（缺失则 `~/.config/opencode/docs/pr-flow.md`）Step 2b 的内容正则表对 notes 内容再校验，命中即 STOP
...
- **代码错误**：`git revert -m 1 <merge-commit>` 生成回滚提交，按 `.opencode/docs/pr-flow.md` 完整流程合回 main，发补丁版本

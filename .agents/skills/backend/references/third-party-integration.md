
# 集成

## 概述

集成外部系统会引入依赖风险。重点在于契约验证、错误处理与优雅降级。

**思想（详见 `references/philosophy.md`）**：外部依赖不可用是常态——系统要**优雅降级**而非崩溃（卡片 6）。集成代码是系统的薄弱点，超时/重试/熔断分层配置是功能的一部分。

## 步骤调整

### 第 1 步 — 需求澄清
询问：
- 要集成哪个外部服务/库
- 哪些数据流入/流出
- 认证方式（API key、OAuth 等）
- 代码库中有哪些可遵循的现有集成模式
- 外部服务不可用时的回退行为

### 第 2 步 — 规格与计划
规格必须包含：
- 集成模式（用适配器/门面层隔离第三方代码）
- 错误处理策略（超时、重试、熔断器）
- 认证处理
- 限流考虑
- 测试策略（mock 还是真实端点）

### 第 3 步 — 权限门禁
向用户明确确认请求许可："我将通过 [适配器/门面层] 集成 [外部服务]，按 [方案] 处理 [认证/错误/限流]。我可以开始吗？"**未经明确许可不得编写代码。**

### 第 4 步 — 开发
- 将外部依赖封装在适配器/抽象层中
- **绝不**将第三方类型直接暴露给代码库其他部分
- 处理所有错误场景：超时、认证失败、畸形响应、限流

### 第 5 步 — 验收
- 尽可能用真实端点测试（不仅是 mock）
- 验证认证流程端到端可用
- 测试错误场景：网络故障、非法凭据、畸形数据
- 验证回退行为
- 验收未过：适配器问题 → 返回第 4 步；集成方案问题 → 返回第 2 步

### 第 6 步 — 兼容性测试
1. **门禁：** 向用户明确确认请求开始兼容性测试的许可。
2. 验证范围：现有调用方/接口在集成后仍然可用；外部服务离线时已有功能正常降级；无数据丢失或损坏。
3. 发现问题：汇总并返回第 2 步。

## 常见借口

| 借口 | 现实 |
|------|------|
| "我以后再补错误处理" | 集成本身就是错误处理。现在就做。 |
| "这个 API 一直返回这个结构" | API 会变。要校验响应。 |
| "我到处直接使用 SDK 就好" | 封装它。SDK 变更时你会感谢自己。 |

## 验证清单

- [ ] 外部服务封装在适配器/抽象层后面
- [ ] 错误处理覆盖：超时、认证失败、错误响应、限流
- [ ] 集成测试通过真实端点验证
- [ ] 服务离线时回退行为正常
- [ ] 核心代码中没有第三方类型泄漏

...
这是一个只读场景。**总指挥模式**：主代理负责澄清范围、派发子代理、汇总报告；每个角色（前端架构师 / 后端架构师 / DevOps 工程师 / QA 测试工程师 / 安全工程师）由独立子代理读取对应审查 reference 并行评审。不编写或修改任何代码。
...
3. 并行调用 当前运行环境支持的并行子代理能力派发子代理，每个 prompt 包含：
   - "以 [角色] 视角读取 [审查 reference] 评审"
   - 评审对象：文件路径 / diff 范围 + 规格文档路径
   - 只读要求：不修改任何文件
   - 输出要求：按审查 reference 的输出格式返回发现报告
4. **一致性检查**——汇总子代理报告：
...
### 第 3 步 — 只读报告
用户已请求评审时直接呈现完整发现；只有修复、额外验证或其他有副作用操作需要另行许可。
...
- **评审期间不得编辑任何文件**
- **不得运行会修改系统的 bash 命令**
- 每个角色必须由子代理按对应审查 reference 评审，主代理不做越权评审
- 聚焦于模式和原则，而非逐行吹毛求疵
- 优先处理可能导致生产事故的发现

## 验证

- [ ] 子代理已按角色派发并读取对应审查 reference
- [ ] 评审覆盖所有相关维度
